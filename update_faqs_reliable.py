...
faq_pattern = re.compile(r"<!-- Preguntas Frecuentes -->.*?<section class=\"service-faq\".*?</section>", re.DOTALL)
cta_pattern = re.compile(r"<!-- CTA Final.*?<section class=\"cta-section.*?</section>", re.DOTALL)
cta_pattern_no_comment = re.compile(r"<section class=\"cta-section.*?</section>", re.DOTALL)
cta_banner_pattern = re.compile(r"<!-- CTA Final Banner -->.*?<section class=\"cta-section.*?</section>", re.DOTALL)
...
for fname, qas in faqs_data.items():
    ...
    # Strip existing blocks aggressively
    content = faq_pattern.sub("", content)
    content = cta_pattern.sub("", content)
    content = cta_banner_pattern.sub("", content)
    content = cta_pattern_no_comment.sub("", content)
    ...
    inject_html = faq_html + cta_banner_html + "\n\n    "
    
    # Inject right before <footer class="footer"> or <!-- Footer -->
    if "<!-- Footer -->" in content:
        content = content.replace("<!-- Footer -->", inject_html + "<!-- Footer -->")
    elif "<footer class=\"footer\">" in content:
        content = content.replace("<footer class=\"footer\">", inject_html + "<footer class=\"footer\">")
